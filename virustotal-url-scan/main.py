...
from threatpatrols.virustotal import VirustotalUrlAnalysis
...
scan_url = GithubInput("url").get()
output_file = GithubInput("output_file", github_summary).get(default="virustotal-url-scan.json")

# ===

virus_total = VirustotalUrlAnalysis(api_key=api_key)
analysis = virus_total.submit_wait_for_analysis(scan_url)
...
github_summary.add_line(f"target: {scan_url}")
github_summary.add_line(f"target_type: url")
...
github_summary.add_line(f"vt_http_response_code: {attributes.get('last_http_response_code')}")
github_summary.add_line(f"vt_http_response_content_sha256: {attributes.get('last_http_response_content_sha256')}")
github_summary.add_line(f"vt_http_response_content_length: {attributes.get('last_http_response_content_length')}")
